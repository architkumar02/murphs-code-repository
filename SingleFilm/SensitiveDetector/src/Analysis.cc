...
void Analysis::EndOfEvent(const G4Event* anEvent){
    ...
    for(int i = 0; i < NUMLAYERS; i++){
        ...
        thisRunTotEGap[NUMLAYERS+1] += thisEventTotEGap[i];
        thisRunTotEAbs[NUMLAYERS+1] += thisEventTotEAbs[i];
        thisRunTotNumSecondaries[NUMLAYERS+1] += thisEventTotNumSecondaries[i];

        // Filling Histograms
        hEDepGapLayer[i]->Fill(thisEventTotEGap[i]/keV);
        hEDepAbsLayer[i]->Fill(thisEventTotEAbs[i]/keV);
    }
    hTotEGap->Fill(thisEventTotEGap[NUMLAYERS+1]);
    hTotEAbs->Fill(thisEventTotEAbs[NUMLAYERS+1]);
