...
A sequence of 10\% \iso{Li}{6}F, 5\% PPO-POPOP films in a PS matrix cast to thickness between 15 and 600 $\mu$m were fabricated and the response was measured from a gamma source as well as a neutron source.
...
\begin{figure}[h]
    ...
    \begin{subfigure}[b]{0.45\figurewidth}
        ...
        \caption{Beta (\iso{Cl}{36}) Spectra Endpoints for PS films}
    \end{subfigure}
    \begin{subfigure}[b]{0.45\figurewidth}
        \includegraphics[width=\textwidth]{PS-5PPOPOPOP_GammaAvg}
        \caption{Gamma (\iso{Co}{60}) Spectra Averages for PS films}
    \end{subfigure}
...
Single collision energy loss spectra provides the probability that that a given collision will result in an energy loss.
